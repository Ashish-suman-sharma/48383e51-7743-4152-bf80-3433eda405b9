import { useContext } from "react"
import { AppThemeContext } from "../../config/AppThemeContext"
import { Typography, Box, Stack, Button, useMediaQuery } from '@mui/material'
import chatIcon from '../../assets/newchat.png'
import { Link } from 'react-router-dom'
import AddCommentIcon from '@mui/icons-material/AddComment';
import CloseIcon from '@mui/icons-material/Close';

export default function NavigationPanel({ resetMessages, dismissMenu }) {

    const { colorMode } = useContext(AppThemeContext)
    const isMobileView = useMediaQuery('(max-width:800px)')

    return (
        <Box>
            {isMobileView && (
                <Button
                    endIcon={<CloseIcon />}
                    sx={{
                        width: 1,
                        justifyContent: 'flex-end',
                        color: colorMode === 'light' ? 'primary.dark' : 'text.primary'
                    }}
                    onClick={dismissMenu}
                >
                    Close
                </Button>
            )}

            <Link to={'/'} style={{ textDecoration: 'none' }}>
                <Stack
                    onClick={() => {
                        resetMessages([])
                        dismissMenu()
                    }}
                    sx={{
                        bgcolor: 'primary.main',
                        '&:hover': {
                            bgcolor: 'primary.bg'
                        }
                    }}
                    direction={'row'}
                    spacing={1}
                    alignItems={'center'}
                    justifyContent={'space-between'}
                    py={2}
                    px={{ xs: 2, md: 3 }}
                >
                    <Stack direction={'row'} gap={1} alignItems={'center'}>
                        <Box
                            component={'img'}
                            src={chatIcon}
                            height={42}
                            width={42}
                            borderRadius={2}
                            boxShadow={4}
                            flexShrink={0}
                        />
                        <Typography
                            variant={'heading'}
                            fontSize={{ xs: 16, md: 20 }}
                            color={'text.primary'}
                        >
                            New Chat
                        </Typography>
                    </Stack>

                    <AddCommentIcon sx={{ color: 'text.primary' }} />
                </Stack>
            </Link>

            <Box p={{ xs: 2, md: 3 }}>
                <Link to={'/history'}>
                    <Button
                        variant="contained"
                        sx={{ width: 1 }}
                        onClick={dismissMenu}
                    >
                        Past Conversations
                    </Button>
                </Link>
            </Box>
        </Box>
    )
}
